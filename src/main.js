...
// DeepL translation via main process (bypasses renderer CSP/network issues)
ipcMain.handle('deepl-translate', async (event, { text, key }) => {
  return new Promise((resolve) => {
    const https = require('https');
    const apiKey = key || store.get('deeplKey', '');
    // Try paid endpoint first, then free
    const tryEndpoint = (host, cb) => {
      const params = new URLSearchParams({ auth_key: apiKey, text, source_lang: 'DE', target_lang: 'EN' }).toString();
      const req = https.request({ hostname: host, path: '/v2/translate', method: 'POST',
        ...
        res.on('end', () => {
          try {
            ...
            if (data.translations && data.translations[0]) cb(null, data.translations[0].text);
            else cb(new Error('No translation: ' + body));
          } catch(e) { cb(e); }
        });
      });
      req.on('error', cb);
